...
`make check` verifies input hashes, rebuilds the analyses, figures and manuscript, then runs linting and tests. `make format` formats R code. The default inputs are in the sibling `../local_elections` checkout ([in-rolls/local_elections](https://github.com/in-rolls/local_elections)). State repositories own collection, parsing and cross-year linkage; the central repository standardizes election events; this repository estimates reservation effects. The UP and Rajasthan source revisions are recorded in `evidence/analysis_inputs.csv` and checked against the central pipeline’s `data/sources.json`, alongside the existing input hashes. Set `LOCAL_ELECTIONS_MASTER` to use a different verified master directory.
